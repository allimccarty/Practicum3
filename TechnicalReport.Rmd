...
p12 <- ggplot(data= otter.try, aes(y=Trout, x=Month, color=Site))+geom_smooth(se=FALSE)
p11 <- ggplot(data= otter, aes(y=Trout.Interp, x=Month, color=Site))+geom_smooth(se=FALSE)
ggarrange(p11, p12, common.legend = TRUE)
```
...
Freq.Dist %>%  kbl(caption = "Frequency Distribution for Total Distance Covered by Otter", label = "Table 2") %>% kable_paper("hover", full_width = F)
```
