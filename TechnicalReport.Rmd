...
>>>>>>> 1d17e730cf294b608069d21b6855bcbd8f70014a
...
As a preliminary step in data analysis, we will check the normality assumption and the distribution plots for all of the variables. We constructed QQPlots for all of the continuous numeric variables, which is included as Figure 2 of the appendix. In addition, we constructed density distributions for continuous variables in Figure 3.  Based on the QQPlots and the density distributions, variables representing activity, trout (the transformed variable), and land deviate from the normal distribution assumption.  All of the aforementioned variables have a strong right-skew. 

We investigated several different methods commonly used to alleviate skews observed in the data. These methods included performing and visualizing log transformations, square root transformations, and cubic root transformations, which are included in Figure 3 of the appendix.The square root transformation was insufficient at alleviating the skew in any of the variables. The cubic root transformation was also insufficient at alleviating the skew in the activity and land variable, but did alleviate the skew in the trout variable.  The log transformation *did help the land and trout variable* (wording?), but was insufficient for the activity variable.  Based on the analysis of these three transformations, we applied a log transformation to the land and trout variables to be included in the model process for this investigation. 

None of the aformentioned transformations were successful in alleviating the skew in the activity variable.  When these transformations were applied, we observed bimodal data, which is incompatible with the normal distribution.  Since these transformations did not help and the application of a transformation to the response variable increases complexity and lessens interpretability of the model, we decided to not perform a transformation on the activity variable.  The log transformation of the aformentioned abnormal variables is included in Figure 4.
