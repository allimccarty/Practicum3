...
descriptices.total <- cbind(descriptives.df.before, descriptives.df.after)

descriptices.total %>% mutate_if(is.numeric, round, digits=2) %>%
...
**explain why we want to look into amoeba**
...
#check im using the right data set
model.dat = otter[-c(21:60),]
#do I use amount of amoeba as a dummy variable 
otter.lm = lm(Amoeba ~ Month + Trout + Caloric + Land, data = model.dat)
#AIC(otter.lm)
# otter.glm = glm(Amoeba ~ Month + Trout.Interp + Caloric + Land, data = otter, family = gaussian)
# AIC(otter.glm)
```
